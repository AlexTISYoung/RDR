...
# 3: output directory. outputs files outprefix_herit.txt and outprefix_n.txt to this directory
...
write.table(sum(!is.na(y_sib$y1y2)),paste(outprefix,'_n.txt',sep=''),quote=F,row.names=F,col.names=F)
...
write.table(summary(r)$coefficients,paste(outprefix,'_herit.txt',sep=''),quote=F)